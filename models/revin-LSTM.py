...
class Model(nn.Module):
    ...
    Encoder: multi-layer LSTM over the (RevIN-normalized) input sequence.
    Forecast: project the last hidden state to the full prediction horizon.
    RevIN:    normalize before the LSTM, denormalize after the projection.
    """
    ...
    def imputation(self, x_enc):
        lstm_out = self.encoder(x_enc)
        return self.projection(lstm_out)  # [B, seq_len, enc_in]
