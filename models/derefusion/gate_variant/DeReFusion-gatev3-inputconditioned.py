...
# ========== 3. Input-Conditioned Gate (v3) ==========
class InputConditionedGate(nn.Module):
    """
    Gate v3: Sample-adaptive gate with bottleneck design.
    
    Uses the normalised input's temporal pattern to produce
    per-sample, per-channel, per-timestep fusion weights.
    Architecture: seq_len → bottleneck → pred_len (per channel).
    Limitation: despite bottleneck constraint, still adds parameters
    that overfit on small financial datasets.
    """

    def __init__(self, seq_len: int, pred_len: int, channels: int, bottleneck: int = 4):
        super().__init__()
        self.gate_net = nn.Sequential(
            nn.Linear(seq_len, bottleneck),
            nn.ReLU(),
            nn.Linear(bottleneck, pred_len),
        )
        self.bias = nn.Parameter(torch.ones(1, pred_len, 1) * -2.0)

    def forward(
        self, x_norm: torch.Tensor, base: torch.Tensor, residual: torch.Tensor
    ) -> torch.Tensor:
        gate_logits = self.gate_net(x_norm.permute(0, 2, 1)).permute(0, 2, 1)
        gate = torch.sigmoid(gate_logits + self.bias)
        return (1.0 - gate) * base + gate * residual


# ========== 4. Top-level Model (InputConditionedGate) ==========
class Model(nn.Module):
    """
    Gate analysis variant v3: InputConditionedGate.
    Full architecture with bottleneck input-conditioned gating.
    """
    ...
    def __init__(self, configs):
        ...
        bottleneck = getattr(configs, "gate_bottleneck", 4)
        ...
        self.gate = InputConditionedGate(
            self.seq_len, self.pred_len, self.enc_in, bottleneck
        )
