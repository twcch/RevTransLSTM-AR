...
# ========== 3. Volatility-Aware Gate (v1) ==========
class VolatilityAwareGate(nn.Module):
    """
    Gate v1: Uses raw input standard deviation as the sole signal.
    
    Extracts std from the RAW input (before RevIN) to guide fusion.
    Limitation: information bottleneck — a single scalar (std) must
    determine per-timestep, per-channel fusion weights.
    """

    def __init__(self, pred_len: int, channels: int):
        super().__init__()
        self.vol_proj = nn.Linear(1, pred_len)
        self.bias = nn.Parameter(torch.ones(1, pred_len, channels) * -2.0)

    def forward(
        self, x_raw: torch.Tensor, base: torch.Tensor, residual: torch.Tensor
    ) -> torch.Tensor:
        vol = torch.std(x_raw, dim=1, unbiased=False, keepdim=True) + 1e-5  # [B, 1, C]
        gate_logits = self.vol_proj(vol.permute(0, 2, 1)).permute(0, 2, 1)   # [B, pred_len, C]
        gate = torch.sigmoid(gate_logits + self.bias)
        return (1.0 - gate) * base + gate * residual


# ========== 4. Top-level Model (VolatilityAwareGate) ==========
class Model(nn.Module):
    """
    Gate analysis variant v1: VolatilityAwareGate.
    Full architecture with volatility-driven gating mechanism.
    """
    ...
    def __init__(self, configs):
        ...
        self.gate = VolatilityAwareGate(self.pred_len, self.enc_in)
