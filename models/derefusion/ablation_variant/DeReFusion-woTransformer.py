...
class LSTMResidualBranch(nn.Module):
    """LSTM-only residual branch (no Transformer)."""

    def __init__(self, enc_in: int, d_model: int, seq_len: int, pred_len: int):
        super().__init__()
        self.lstm = nn.LSTM(
            input_size=enc_in, hidden_size=d_model,
    ...
    def forward(self, x: torch.Tensor) -> torch.Tensor:
        ...
        out = self.time_proj(lstm_out.permute(0, 2, 1)).permute(0, 2, 1)
        return self.channel_proj(out)


# ========== 3. Top-level Model (woTransformer) ==========
class Model(nn.Module):
    """
    Ablation: woTransformer — DLinear + LSTM-only residual + direct add.
    Tests the contribution of the Transformer component in the hybrid branch.
    """
