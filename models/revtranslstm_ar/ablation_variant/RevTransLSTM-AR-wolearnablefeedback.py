...
class Model(nn.Module):
    """
    Ablation: wolearnablefeedback — 移除「可學習」feedback 投影的 RevTransLSTM-AR。

    保留 AR feedback 結構與 RevIN／Cross-Attention，但將可學習的
    φ(·) = nn.Linear(c_out, d_model) 替換為固定的 ones buffer W (shape: [D, c_out])，
    每步以 `u_t = C_t + W·y'_t` 餵入下一步 LSTM。用於與 main 模型對照，隔離出
    「學習 feedback 對齊」相對於「僅保留 feedback 結構」的貢獻；與 woarfeedback
    (完全移除 feedback) 互補。
    """
    ...
    def __init__(self, configs):
        ...
        # 非可學習 feedback 對齊矩陣 W：以 ones buffer 取代可學習的 nn.Linear
        # 形狀 [d_model, c_out]，於 forward 中以 pred @ W.t() 廣播至 d_model
    ...
    def forward(
        ...
        # 4. 自迴歸解碼：feedback 改用固定 W (ones) 廣播，無可學習參數參與對齊
        for i in range(self.pred_len):
            ...
            # u_t = C_t + W·y'_t；W 為非可學習 ones buffer，將 c_out 廣播到 d_model
